...
export class DictionaryPortalAtDiscord {
  ...
  async initialize(userId: number) {
    ...
    if (this.message) {
      ...
      message_c = await this.interaction.editReply(
        await this.generateMainMenu()
      );
    }
    ...
    this.collector.once("end", async () => {
      this.interaction.editReply({
        components: [],
        content: "Время ожидания ответа на запрос истекло",
      });
      setTimeout(async () => {
  ...
  async generateMainMenu() {
    const embeds = [
      new EmbedBuilder()
        .setTitle(`Словарь ${this.dictionary.language.source}-${this.dictionary.language.target}`)
        .setFooter({
          text: `Версия бота: ${process.env.version}`,
        ...
        .setDescription(
          `В этом словаре ${this.dictionary.sets.length} наборов, в них сохранено ${this.dictionary.flashcards.length} слов`
        )
        .addFields(
          ...this.dictionary.folders.map((folder) => {
            return {
              name: `${folder.name} (${folder.sets.length} наборов)`,
              value: `${this.dictionary.sets
                .filter((set) => set.folder === folder.id)
                .map(
                  (set, i) =>
                    `${i}. **${set.name}** (${set.flashcards.length} слов.)`
                )
                .join(", ")}`,
            };
          })
  ...
  setsPaginationUtil(page: number, max: number = 23) {
    const pages = Math.ceil(
      this.dictionary.sets.filter(
        (set) =>
          this.dictionary.folders.find((folder) => folder.id === set.folder)
    ...
    row.addComponents(
      new ButtonBuilder()
        .setCustomId(`sets:previous:${page - 1}`)
        .setStyle(ButtonStyle.Primary)
        .setDisabled(page === 1)
        .setLabel("<")
    );
    ...
    for (let i = 1, iSum = 0; iSum + i <= max; i++) {
      ...
      if (i === page) {
        row.addComponents(
          new ButtonBuilder()
            .setCustomId(`sets:page:${setNumber}`)
            .setStyle(
              setNumber === page ? ButtonStyle.Primary : ButtonStyle.Secondary
            )
            .setDisabled(setNumber === page)
            .setLabel(`${setNumber}`)
        );
      }
    ...
    const embeds = [
      new EmbedBuilder()
        .setFooter({
          text: `Всего найдено наборов ${this.dictionary.sets.length} | Страница ${page} из ${pages}`,
        })
        .setTimestamp(),
    ];
    ...
    embeds[0].setDescription(
      this.dictionary.sets
        .slice(max * (page - 1), max * page)
        .map((set, index) => {
