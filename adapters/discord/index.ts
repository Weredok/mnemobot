...
client.on("interactionCreate", async (interaction) => {
    if (interaction.isCommand()) {
        ...
        if (interaction.commandName === "dictionary") {
            ...
            const message = await interaction.reply({ content: 'test', components: [row] });
            ...
            collector.on('collect', async i => {
                if (i.customId === 'primary') {
                    ...
                    const userId = (await User.findOneBy({ discordIDS: interaction.user.id })).id;

                    const dictionary = await Dictionary.findOneBy({ userId });
                    const flashcard = new Flashcard();
                    flashcard.set = "020202";
                    flashcard.createdAt = Date.now();
                    flashcard.lastReviewed = Date.now();
                    flashcard.user = dictionary.user.id;
                    flashcard.front[0] = "Привет, мир!"
                    flashcard.back[0] = "Hello, world!"
                    flashcard.quality = [5]
                    ...
                    session.user = dictionary.user.id;
                    await session.save();

                    await dictionary.syncronize();
                    await dictionary.flashcards.push(flashcard);
                    await dictionary.user.sessions.push(session.id);

                    const prefferences = new Preferences();
                    prefferences.init(dictionary.user);

                    await prefferences.save();
                    dictionary.preferences = prefferences;


                    const review = new reviewWord(flashcard, session, dictionary, dictionary.user, i);
        ...
        if (interaction.commandName === "start") {
            ...
            const components = [
                new ActionRowBuilder<ButtonBuilder>().addComponents(
                    new ButtonBuilder().setCustomId("settings").setLabel("Настройки").setStyle(ButtonStyle.Primary),
                    new ButtonBuilder().setCustomId("dictionary").setLabel("Словарь").setStyle(ButtonStyle.Primary),
                    new ButtonBuilder().setCustomId("create").setLabel("Создать").setStyle(ButtonStyle.Success),
                    new ButtonBuilder().setCustomId("interactive").setLabel("Интерактив").setStyle(ButtonStyle.Primary),
            ...
            collector.on("collect", async (interaction) => {
                ...
                switch (int.type) {
                    case ComponentType.Button: {
                        ...
                        const dictionary = await Dictionary.findOneBy({ userId: user.id, language: { source: ILike(data) } });
                        await dictionary.syncronize();


                        switch (name as "settings" | "dictionary" | "create" | "interactive" | "sets" | "language" | "filter" | "interactive" | "frequency" | "dateOfCreation" | "length" | "forgotten" | "polysemitic" | "page" | "previous" | "next") {
                            ...
                            case "forgotten":
                            case "polysemitic": {
                                let value: undefined | boolean = dictionary.preferences.dictionaryFilters[int.customId.split(':')[0]];
                                switch (value) {
                                    case true:
                                        value = false
                                        break;
                            ...
                            case "language": {
                                if (!data) {
                                    ...
                                    const embeds = [new EmbedBuilder().setTitle("Выберите язык, с которым хотите работать сейчас")];
                                    ...
                                    for (let i = 0; i < dictionary.user.languages.length; i++) {
                                        ...
                                        row.addComponents(
                                            new ButtonBuilder().setCustomId(`dictionary:${dictionary.user.languages[i]}`).setLabel(dictionary.user.languages[i]).setStyle(ButtonStyle.Success)
                                        );
                                        embeds[0].addFields({
                                            name: dictionary.user.languages[i],
                                            value: `Словарь на n (в будущем доделаю) слов`
                                        })
                                        if (row.components.length === 5 || i === dictionary.user.languages.length - 1) {
                                            components.push(row);
                                    ...
                                    await interaction.showModal(new ModalBuilder().setCustomId(`dictionary:newlanguage`).setTitle("Выбор нового языка для изучения").addComponents(
                                        new ActionRowBuilder<TextInputBuilder>().addComponents(
                                            new TextInputBuilder().setCustomId("language").setLabel("Название языка").setStyle(TextInputStyle.Short).setRequired(true).setMinLength(5).setMaxLength(25)
                                        )
                                    ))

                                    await interaction.awaitModalSubmit({ time: dictionary.preferences.idleTimeout }).then(async (int) => {
                                        const newDictionary = new Dictionary();

                                        await setupNewLanguage(int, newDictionary, int.fields.getTextInputValue("language"))
                                    });
                                }
                                break;
                            }
